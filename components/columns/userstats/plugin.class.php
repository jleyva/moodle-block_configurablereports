...
class plugin_userstats extends plugin_base{
	...
	function execute($data,$row,$user,$courseid,$starttime=0,$endtime=0){
		...
		if($data->stat == 'coursededicationtime'){
			...
			if ($logs = get_records_select("log", $sql, "time ASC", "id,time")) {
		...
		$sql = "SELECT SUM($total) as total FROM {$CFG->prefix}stats_user_daily WHERE stattype = '$stattype' AND userid = {$row->id}";
