...
class plugin_userfield extends plugin_base{
	...
	function execute($data,$row,$user,$courseid,$starttime=0,$endtime=0){
		...
		if(strpos($data->column,'profile_') === 0){
			if($profiledata = get_records_sql("SELECT d.*, f.shortname, f.datatype FROM {$CFG->prefix}user_info_data d ,{$CFG->prefix}user_info_field f 
							WHERE f.id = d.fieldid AND d.userid = $row->id")){
