...
function cr_get_my_reports($courseid, $userid, $allcourses=true){
	...
	if(has_capability('block/configurable_reports:managereports', $context, $userid)){
		if($courseid == SITEID && $allcourses)
			$reports = get_records('block_configurable_reports_report','','','name ASC');
		else
			$reports = get_records('block_configurable_reports_report','courseid',$courseid,'name ASC');
	}
	else{
		$reports = get_records_select('block_configurable_reports_report','ownerid = '.$userid.' AND courseid = '.$courseid.' ORDER BY name ASC');		
	}
	return $reports;
}
  
 function cr_serialize($var){
	return serialize(urlencode_recursive(addslashes_recursive($var)));
 }
